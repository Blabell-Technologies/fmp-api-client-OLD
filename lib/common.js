...
const url = (path, query) => {
  ...
  const base = "http://192.168.1.45:3000/";
...
/** Separa y añade datos de un Objeto a un FormData
  ...
  const parse = (data) => {
    ...
    for (var entry in data) { if (only_setted(data[entry])) formData.append(entry, data[entry]); }
    return formData;
  }

module.exports = { only_setted, is_set, url, verify, parse };
