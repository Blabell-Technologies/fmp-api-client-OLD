 ...
 * pet_animal: string, 
 * pet_race: string, 
 * dissappearance_date: Date,
 * dissappearance_place: { coordinates: [number, number], address: string }
 * details: string,
 * picture: string 
 * }} PetMiniInformation
 * 
...
class Pets extends Sender {
   ...
   *
   * @returns {PetMiniInformation[]} Resumen de informacion de errores
   */
  async nearby(opt = {}) {
   ...
   * @param {string} opt.dissappearance_date - Fecha y hora de desaparición
   * @param {string} opt.dissappearance_place - Lugar de desaparición en coordenadas separadas por coma
  ...
  async add(opt = {}) {
    ...
    try {
      common.is_set(
        opt.pet_animal,
        opt.pet_name,
        opt.dissappearance_date,
        opt.dissappearance_place,
        opt.details,
        opt.pet_photo_0,
        opt.owner_name,
        opt.owner_phone,
    ...
    // Coordenadas
    opt.dissappearance_place = common.verify(
      opt.dissappearance_place,
      "string"
    );
    if (!/^(-?\d+(\.\d+)?),\s*(-?\d+(\.\d+)?)$/g.test(opt.dissappearance_place))
    ...
    // Cronológico
    opt.dissappearance_date = common.verify(opt.dissappearance_date, "string");
    if (
