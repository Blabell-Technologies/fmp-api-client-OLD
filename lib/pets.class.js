  ...
  async edit(id, opt = {}) {
    ...
    if (opt.owner_phone != undefined) {
      ...
      if (opt.owner_phone.unset != undefined) {
        opt.owner_phone.unset.map(phone => {
          const validation = common.phone_validation(phone);
          if (!validation) return false;
          composed.push(`unset[${phone}]`)
        })
      }
      ...
      if (opt.owner_phone.add != undefined) {
        opt.owner_phone.add.map(phone => {
          const validation = common.phone_validation(phone);
          if (!validation) return false;
          composed.push(`add[${phone}]`)
        })
      }
      
      if (opt.owner_phone.replace != undefined) {
        opt.owner_phone.replace.map(phone => {
          const validation_index = common.phone_validation(phone[0]);
          const validation_replace = common.phone_validation(phone[1]);
          if (!validation_index || !validation_replace) return false;
          composed.push(`replace[${phone.join(', ')}]`)
        })
      }

      opt.owner_phone = composed.join(',');
      console.log(opt.owner_phone);
